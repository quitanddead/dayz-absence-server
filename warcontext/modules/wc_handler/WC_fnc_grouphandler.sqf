		...
		_group = _this select 0;
		{
			...
			_x setVehicleInit "this addAction ['<t color=''#ffcb00''>Hands UP</t>', '\z\addons\dayz_code\warcontext\actions\WC_fnc_dohandsup.sqf',[],-1,false, true];";
			_x setVehicleInit "this addAction ['<t color=''#ffcb00''>Follow me</t>', '\z\addons\dayz_code\warcontext\actions\WC_fnc_dofollowme.sqf',[],-1,false, true];";
			...
			_x addeventhandler ['killed', {
				private ["_name", "_increase", "_ammo"];
				_name = currentMagazine (_this select 1);
				_ammo = currentMagazine (_this select 1);
			...
			_x addEventHandler ['Fired', '
				...
				_ammo = currentMagazine (_this select 0);
				_name = getText (configFile >> "CfgMagazines" >> _name >> "displayNameShort");
				_ammo = getText (configFile >> "CfgMagazines" >> _ammo >> "ammo");
				_sammo = getnumber (configfile >> "CfgMagazines" >> _ammo >> "ace_suppressed");
				_increase = ceil(random (5));
			...
			_x addEventHandler ['HandleDamage', {
				if(side(_this select 3) in [west, civilian]) then {
					...
					_ammo = currentMagazine (_this select 3);
					_name = getText (configFile >> "CfgMagazines" >> _name >> "displayNameShort");
					_ammo = getText (configFile >> "CfgMagazines" >> _ammo >> "ammo");
					_sammo = getnumber (configfile >> "CfgMagazines" >> _ammo >> "ace_suppressed");
					...
					(_this select 0) setdamage (0.2 + (random 0.5));
				};
			}];
