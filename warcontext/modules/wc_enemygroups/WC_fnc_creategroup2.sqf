	...
	if (_motorized) then {
		_position2 = [_marker3, "onground", "onflat"] call WC_fnc_createpositioninmarker;
		_arrayofvehicle = [_position2, 0, _typeofgroup, east] call BIS_fnc_spawnVehicle;
		sleep 0.05;
		...
		diag_log format ["WARCONTEXT: CREATING VEHICLE %2 IN ZONE %1", _marker3, _typeofgroup];
		...
		_sizeofgroup = 3;
		_position2 = [_marker3, "onground"] call WC_fnc_createpositioninmarker;
		_group = [_typeofgroup, east, _position2] call WC_fnc_popgroup;
		diag_log format ["WARCONTEXT: CREATING A GROUP %2 IN ZONE %1 OF SIZE %3", _marker3, _typeofgroup, _sizeofgroup];
	};
...
//					};
			...
			wcgarbage = [_vehicle, _marker3, 'showmarker'] execVM '\z\addons\dayz_server\extern\ups.sqf';
		//};
		...
		if(random 1 > 0.4) then {
			_list = nearestObjects [_position2, ["house"] , 70];
			if(count _list > 10) then {
				wcgarbage = [(leader _group), _marker3, 'showmarker', 'fortify'] spawn EXT_fnc_upsmon;
				//wcgarbage = [_group, (position(leader _group)), wcdistance] spawn WC_fnc_patrol;
			} else {
				wcgarbage = [(leader _group), _marker3, 'showmarker'] spawn EXT_fnc_upsmon;
				//wcgarbage = [_group, (position(leader _group)), wcdistance] spawn WC_fnc_patrol;
			};
		} else {
			wcgarbage = [(leader _group), _marker3, 'showmarker'] spawn EXT_fnc_upsmon;
