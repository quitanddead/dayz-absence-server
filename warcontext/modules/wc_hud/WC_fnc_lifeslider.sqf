	...
	while {true} do {
		...
		if(vehicle player != player) then {
			...
			if(wcrankchanged or wcrankactivate) then {
				if(format["%1", wcanim] == "") then {
					_text = "<t size='1.35' shadow='true' color='#EEEEEE'>Ранг Команды [""TAB""]<br/></t>";
					{	
							_rank = _x;
							{
								if(!(format["%1", name _x] == "") and !(format["%1", name _x] == "Error: No unit")) then {
					...
					if(_key == "") then {
						_text = "<t size='1.5'>Нажмите [""TAB""] чтобы пропустить видео</t><br/><t size='1.5'>Нажмите [""Cancel Action""] чтобы сменить музыку</t>";
					} else {
						_text = format["<t size='1.5'>Нажмите [""TAB""] для пропуска видео</t><br/><t size='1.5'>Нажмите [%1] чтобы сменить музыку</t>", _key];
					};
				};
			} else {
				_text ="";							
		...
		_text = _text + format ["День %1 %2<br/>", wcday];
		if(wcradioalive) then {	
			if(wcalert > 99) then { 
				wcalert = 100;
				_text = _text + format ["<t color='#CC0000'>Заметность: %1</t><br/>", format["%1", wcalert] +"%"];
				if!(wcdetected) then {
				...
				_text = _text + format ["Заметность: %1<br/>", format["%1", (ceil wcalert)] +"%"];
				wcdetected = false;
			};
		} else {
			_text = _text + "Заметность: Радио уничтожено<br/>";
		};
		
		_ammo = currentMagazine player;
		...
		if(_name == "SD" || _ammo == "B_9x39_SP5" || _ammo == "ACE_B_9x39_SP6") then {
			_text = _text + "Режим: Скрытность<br/>";
		} else {
			_text = _text + "Режим: Бой<br/>"
		};
		...
		if(wckindofserver != 3) then {
			...
			_text = _text + format ["Не в команде - уровень: %1<br/>", wclevel];
		};
