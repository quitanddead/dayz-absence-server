	...
	while {(format["%1", _position2] == "[0,0,0]" or _invalidate)} do {
		_position2 = [0,0,0];
		_invalidate = false;
		...
		_position2 = [_newx, _newy];
		if (_invillage) then {
			_nearestlocation = nearestLocation [_position2, "NameVillage"];
			_position2 = position _nearestlocation;
		};
		if (_incity) then {
			_nearestlocation = nearestLocation [_position2, "NameCity"];
			_position2 = position _nearestlocation;
		};
		if (_incitycapital) then {
			_nearestlocation = nearestLocation [_position2, "NameCityCapital"];
			_position2 = position _nearestlocation;
		};
		if (_onmountain) then {
			_position2 = [_position2] call WC_fnc_getterraformvariance;
		};
		if(_empty) then {
			_position2 = _position2 findEmptyPosition [_emptysizezone, 300];
		};
		if (_onflat) then {
			_isflat = [];
			_isflat = _position2 isflatempty [2, 0, 0.2, 2, 0, false];  
			if (count _isflat == 0) then { _position2 = [0,0,0]; _invalidate = true;};
		};
		if (_onflatforbase) then {
			_isflat = [];
			_isflat = _position2 isflatempty [20, 0, 0.1, 20, 0, false];  
			if (count _isflat < 1) then { _position2 = [0,0,0]; _invalidate = true;};
		};
		if (_onroad) then {
			if (!isOnRoad _position2) then { _position2 = [0,0,0]; _invalidate = true;};
		} else {
			if(count(_position2 nearRoads 15) > 0) then {
				_position2 = [0,0,0]; _invalidate = true;
			};
		};
		if (_onhill) then {
			_nearestlocation = nearestLocation [_position2, "Hill"];
			_position2 = position _nearestlocation;
		};
		if (_onmount) then {
			_nearestlocation = nearestLocation [_position2, "Mount"];
			_position2 = position _nearestlocation;
		};
		if(_onground) then {
			// Position can not be at less 50 meters of water
			_gridofposition = [_position2, 50] call WC_fnc_creategridofposition;
			{
		...
		if(_count > 50) then { _onflat = false; _emptysizezone = 3;
		 //diag_log "WARCONTEXT: POSITION LOOP exit over 50";
		 };
		if(_count > 100) then { _position2 = [1,1,0]; _invalidate = false;
		 //diag_log "WARCONTEXT: POSITION LOOP exit over 100";
		 };
		sleep 0.05;
	};
