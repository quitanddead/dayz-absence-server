	...
	while { (wclevel < wclevelmax) and (wcteamscore < wcscorelimitmax)} do {
		...
		if(isnil text _location) then {
			...
			wcmessageW = [format["Задание %1", wcmissioncount], format[localize "STR_WC_MESSAGENEAR", text _city], localize "STR_WC_MESSAGETAKISTANLOCALISED"];
		} else {
			wcmessageW = [format["Задание %1", wcmissioncount], format[localize "STR_WC_MESSAGEGOTO", text _location], localize "STR_WC_MESSAGETAKISTANLOCALISED"];
		};
		["wcmessageW","client"] call WC_fnc_publicvariable;
		...
		"operationtext" setMarkerText "Задание закончено. Покиньте зону.";
		...
		wcmessageW = ["Потери", format["%1 Врагов убито", wcnumberofkilledofmissionE]];
		["wcmessageW", "client"] call WC_fnc_publicvariable;

		sleep 0.5;

		wcmessageW = ["Потери", format["%1 Наших убито", wcnumberofkilledofmissionW]];
		["wcmessageW", "client"] call WC_fnc_publicvariable;

		sleep 0.5;

		wcmessageW = ["Потери", format["%1 гражданских убито", wcnumberofkilledofmissionC]];
		["wcmessageW", "client"] call WC_fnc_publicvariable;

		sleep 0.5;

		wcmessageW = ["Потери", format["%1 Техники уничтожено", wcnumberofkilledofmissionV]];
		["wcmessageW", "client"] call WC_fnc_publicvariable;
		...
		wcmessageW = ["Оценка военных", format["%1", _fame]];
		["wcmessageW", "client"] call WC_fnc_publicvariable;
