...
//	wcgarbage = [] spawn WC_fnc_lifeslider;
	...
	wcactionmenuoption = wcvehicle addAction ['<t color=''#ff4500''>Mission Info</t>', 'warcontext\dialogs\WC_fnc_createmenumissioninfo.sqf',[],5,false]; enableEnvironment false;", 
		...
		while { true } do {
			...
			if (_oldlevel != wcteamlevel) then {
				if (_oldlevel > wcteamlevel) then {
					...
					_message =[localize "STR_WC_MESSAGETEAMPROMOTED", format["до %1 !", _teampromote]];
					wcgarbage = _message spawn EXT_fnc_infotext;
				} else {
					_oldlevel = wcteamlevel;
					playsound "drum";
					_teampromote = localize format["STR_WC_TEAM%1", wcteamlevel];
					_message =[localize "STR_WC_MESSAGETEAMDEGRADED", format["до %1 !", _teampromote]];
					wcgarbage =  _message spawn EXT_fnc_infotext;
				};
			};
			sleep 5;
	...
	wcgarbage = [] spawn {
		while { true } do {
			...
			if(wcbonus > 10000) then {
				_message =["Вы выиграли", "10 очков за коммандную игру"];
				wcgarbage = _message spawn EXT_fnc_infotext;
				wcbonus = 0;
				...
				if(count _units == 2) then {
					{
						if((_x != player) and ((animationState _x) in _animation)) then {
							if(isnil "wcdragg") then {
								wcdragg = player addAction ["<t color='#dddd00'>Тащить игрока</t>", "warcontext\actions\WC_fnc_dodrag.sqf",[],-1,false];
							};	
							if(wcdragged) then {
		...
		while { true } do {
			if((wceverybodymedic == 1) and !(typeOf player in wcmedicclass)) then {
				...
				if(!(isnull _injured) and (isnil "wcmedicmenu"))  then {
					wcmedicmenu = player addAction ["<t color='#dddd00'>Лечить</t>", "warcontext\actions\WC_fnc_doheal.sqf",[_injured],6,false];
				} else {
	...
	["wcplayeraddscore", "server"] call WC_fnc_publicvariable;
//call compile preprocessfilelinenumbers "modules\rmm_nomad\main.sqf";